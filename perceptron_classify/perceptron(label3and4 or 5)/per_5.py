    ...
    for file_name in files:
        if file_name.endswith(".txt"):
            ...
            for each_lines in lines:
                ...
                new_message = message_s("Score:3or4")
                value = int(token_arr[0].split(".")[0])
                if value >= 3:
                    if value != 5:
                        dislike_message_count +=1
                    else:
                        new_message.changeStyle("Score:5")
...
for i in range(0,20):
    ...
    for j in range(0, len(message_list)):
        ...
        if new_message.style in "Score:5":
            label_value = 1
            alpha_value = 0
        ...
        elif new_message.style in "Score:3or4":
            label_value = -1
            alpha_value = 0
            new_dict_spam = new_message.token_self_dict
            for every_token in new_dict_spam:
...
model_file_data.write("score3or4_message_count %s" %dislike_message_count)
model_file_data.write("\n")
model_file_data.write("score5_message_count %s" %like_message_count)
model_file_data.write("\n")
