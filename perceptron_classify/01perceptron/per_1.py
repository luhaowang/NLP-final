    ...
    for file_name in files:
        if file_name.endswith(".txt"):
            ...
            for each_lines in lines:
                ...
                new_message = message_s("Score:1")
                value = int(token_arr[0].split(".")[0])
                if value < 3:
                    if value == 1:
                        dislike_message_count +=1
                    else:
                        new_message.changeStyle("Score:2")
...
for i in range(0,20):
    ...
    for j in range(0, len(message_list)):
        ...
        if new_message.style in "Score:2":
            label_value = 1
            alpha_value = 0
        ...
        elif new_message.style in "Score:1":
            label_value = -1
            alpha_value = 0
            new_dict_spam = new_message.token_self_dict
            for every_token in new_dict_spam:
...
model_file_data.write("score0_message_count %s" %dislike_message_count)
model_file_data.write("\n")
model_file_data.write("score1_message_count %s" %like_message_count)
model_file_data.write("\n")
